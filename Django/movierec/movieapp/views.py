...
def recommend(request):
    ...
    sys.stdout.flush()
    movie_data = pd.read_csv("./data/movie_data_final2.csv", index_col=0)
    ...
    movie_data['title'] = movie_data['title'].str.replace("[^\w]", " ")
    ...
    print("검색중.....")
    with open("./models/word2vec.pickle", "rb") as fr:
        embedding_model = pickle.load(fr)
    with open("./models/tfidf.pickle", "rb") as fr:
        Tfidf = pickle.load(fr)
    Tfidf_matrix = mmread('./models/word2VecModel_final.mtx')
...
def nmf_recommend(request):
    ...
    def recommendation(embedding_model, key_word, movie_data, Tfidf, Tfidf_matrix):
        ...
        simScores = sorted(simScores, key=lambda x: x[1],
                           reverse=True)
        simScores = simScores[0:30]
        movieidx = [i[0] for i in simScores]
        RecMovielist = movie_data.iloc[movieidx].reset_index(drop=True)
        topic_mode = mode(RecMovielist['nmf_topic'])[0]
        res = []
        for i in movieidx:
            if movie_data['nmf_topic'].loc[i] == topic_mode[0]:
                ...
                if len(res) == 10:
                    break
        RecMovielist = movie_data.iloc[res].reset_index(drop=True)
        #     fig = plt.figure(figsize=(20,20))
        #     for index, row in RecMovielist.iterrows():
        #         response = requests.get(row['image_link'])
        #         img = Image.open(BytesIO(response.content))
    ...
    with open("./models/tfidf.pickle", "rb") as fr:
        Tfidf = pickle.load(fr)
    Tfidf_matrix = mmread('./models/word2VecModel_final.mtx')
    data = recommendation(embedding_model, key_word2, movie_data, Tfidf, Tfidf_matrix)
    ...
    return render(request, 'nmf_recommend.html', context)